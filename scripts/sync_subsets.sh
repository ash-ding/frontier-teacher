#!/usr/bin/env bash
# Push data/subsets/ from this checkout to every compute node, then verify by
...
for h in $HOSTS; do
  echo "--- $h"
  ssh -o ConnectTimeout=30 "$h" 'mkdir -p ~/code/frontier-teacher/data/subsets ~/data/frontier-teacher'
  scp -q -o ConnectTimeout=60 -r "$SRC"/* "$h:~/code/frontier-teacher/data/subsets/"
  ssh -o ConnectTimeout=30 "$h" '
    rm -rf ~/data/frontier-teacher/subsets
    mkdir -p ~/data/frontier-teacher/subsets
    cp -r ~/code/frontier-teacher/data/subsets/* ~/data/frontier-teacher/subsets/'
  remote=$(ssh -o ConnectTimeout=30 "$h" \
    "cd ~/code/frontier-teacher/data/subsets && find . -name '*.jsonl' | sed 's|\./||' | sort | \
