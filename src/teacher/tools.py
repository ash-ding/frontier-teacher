...
def _trajectory_summary(run_dir, step):
    """One compact line per completed prior step, read from its result.json."""
    lines = []
    for k in range(step):
        ...
        try:
            ...
            continue
        bit = f"  step {k}: decision={r.get('decision')} status={r.get('status')}"
        if r.get("eval"):
            e = r["eval"]
            bit += (f" | eval n={e.get('n_problems')} pass@1={e.get('pass_at_1')}"
                    f" pass@k={e.get('pass_at_k')} trunc={e.get('truncation_rate')}"
                    f" no_answer={e.get('no_answer_rate')}")
        if r.get("train"):
            t = r["train"]
            bit += f" | train n={t.get('n_problems')} metrics={t.get('metrics')}"
        lines.append(bit)
    return "\n".join(lines) if lines else "  (no prior steps -- this is step 0)"


def render_context(run_dir, step, config, latest_ckpt, repo_root):
    """Build the teacher's per-step task message (a plain string).

    Mounts, by absolute path so the teacher can Read/cat/grep them:
      * the curated reference training data (comparison point, context only),
      * the verifier + training/eval source (read-only),
      * the FULL trajectory of every prior step (decision, data, logs, results).
    Imposes no curriculum strategy -- the standing role/protocol lives in the
    system prompt; this message only supplies the step number and the paths.
    """
    ...
    for k in range(step):
        ...
        for fn in ("decision.json", "data.jsonl", "result.json",
                   "eval.summary.json", "train.log", "eval.log", "teacher.log"):
            if (sd / fn).exists():
                prior_paths.append(str(sd / fn))

    msg = [
        f"You are on STEP {step} of the curriculum loop.",
        "",
        "Make ONE decision (evaluation | train) and write your two files into your",
        f"current working directory ({run_dir / f'step_{step}'}):",
        "  - decision.json",
        "  - data.jsonl",
        f'Use step number {step} in decision.json: {{"step": {step}, "decision": "..."}}',
        "",
        "The student's current checkpoint (what an evaluation would measure, and",
        f"what a train step would update) is:\n  {latest_ckpt}",
        "",
        ...
        *[f"  {p}" for p in ro],
        "",
        "The FULL trajectory of every prior step (read these to decide what to do",
        "next):",
        _trajectory_summary(run_dir, step),
        "",
        "Prior-step files you may open directly:",
...
def run_evaluation(step_dir, step, model_path, config, repo_root, timeout_s):
    ...
    Reuses evaluate.py unmodified: `--limit 0` (uncapped), `--save-generations`,
    `--out step_dir`, `--name teacher_step<N>`. Copies the tag-named summary/records
    to the canonical eval.summary.json / eval.records.jsonl in the step dir.
    Returns a dict of eval stats (weights unchanged).
    """
    step_dir = Path(step_dir)
    step_data = step_dir / "data.jsonl"
    eval_cfg = write_eval_config(step_dir, step, model_path, config, step_data, repo_root)
    tag = f"teacher_step{step}__teacher_eval"
